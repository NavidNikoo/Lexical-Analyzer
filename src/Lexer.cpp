...
void Lexer::skipSpace() {
    // keep looping because we may hit whitespace, then a comment, then more whitespace, etc.
    while (true) {
        // 1) whitespace
        while (!eof && std::isspace(static_cast<unsigned char>(current))) advance();
        if (eof) return;

        // 2) quoted comment: " ... "   (assignment note: treat as comment, ignore entirely)
        if (current == '"') {
            advance(); // skip opening "
            while (!eof && current != '"') {
                advance();
            }
            if (!eof && current == '"') advance();  // skip closing "
            // loop again: there might be more whitespace/comments after
            continue;
        }
        break;
    }
